...
# from threading import Thread
# from PyQt6.QtWidgets import QApplication, QMainWindow, QPushButton, QVBoxLayout, QWidget, QDialog, QLabel
# from PyQt6.QtGui import QIcon, QMouseEvent, QMovie, QRegion, QBitmap, QPainterPath
# from PyQt6.QtCore import QTimer, QSize, QRect, Qt, QPropertyAnimation, QThread
...
# # Создаем круглую маску для GIF
# def create_circular_mask(widget, size):
#     path = QPainterPath()
#     path.addEllipse(0, 0, size, size)  # Создаем круглый путь
#     region = QRegion(path.toFillPolygon().toPolygon())  # Преобразуем путь в полигон и создаем регион
#     widget.setMask(region)  # Применяем маску для виджета
...
#         # Добавляем QLabel для отображения GIF анимации поверх кнопки
#         self.gif_label = QLabel(self.assistant_button)
#         self.gif_movie = QMovie("icons/microphonee.gif")
#         self.gif_label.setMovie(self.gif_movie)
#         self.gif_label.setScaledContents(True)  # Масштабирование GIF в зависимости от размера QLabel
#         self.gif_label.hide()  # Изначально скрываем GIF
...
#     def centralize_button(self):
#         """Функция для центрирования кнопки голосового помощника и изменения её размера в зависимости от размеров экрана."""
#         button_size = min(self.width(), self.height()) // 2  # Размер кнопки = 1/4 от минимального размера окна
#         self.assistant_button.setFixedSize(button_size, button_size)
#         self.assistant_button.setIconSize(QSize(button_size // 2, button_size // 2))  # Размер иконки 1/2 от кнопки
#         self.assistant_button.setStyleSheet(f"""
#             QPushButton {{
...
#         # Настраиваем GIF размером под кнопку
#         self.gif_label.setFixedSize(button_size, button_size)
        
#         # Создаем круглую маску для GIF
#         create_circular_mask(self.gif_label, button_size)
...
#             # Отключаем ассистент, меняем иконку на перечеркнутый микрофон
#             self.assistant_button.setIcon(QIcon("icons/microphone_off_red.png"))
#             self.gif_movie.stop()  # Останавливаем анимацию GIF
#             self.gif_label.hide()  # Скрываем GIF

#             self.statusBar().showMessage("Voice Assistant deactivated", 2000)
#             os.environ['MIC'] = '0'  # Отключаем ассистент через переменную окружения
#             self.assistant_thread.quit()  # Останавливаем поток ассистента
#         else:
#             # Включаем ассистент, показываем GIF вместо иконки
#             self.assistant_button.setIcon(QIcon())  # Убираем иконку
#             self.gif_label.show()  # Показываем GIF
#             self.gif_movie.start()  # Запускаем анимацию GIF
